...
- (void) setUpLayers {
	...
	annotationPage = [[CALayer alloc] init];
	annotationPage.masksToBounds = YES;
	annotationPage.contentsGravity = kCAGravityLeft;
	annotationPage.backgroundColor = [[UIColor clearColor] CGColor];
...
//	annotationPageReverse = [[CALayer alloc] init];
//	annotationPageReverse.backgroundColor = [[UIColor clearColor] CGColor];
	...
	bottomAnnotationPage = [[CALayer alloc] init];
	bottomAnnotationPage.backgroundColor = [[UIColor clearColor] CGColor];
	...
	[topPage addSublayer:annotationPage];
	[topPage addSublayer:topPageShadow];
	[topPage addSublayer:topPageOverlay];
	//[topPageReverse addSublayer:annotationPageReverse];
	...
	[bottomPage addSublayer:bottomAnnotationPage];
...
-(void) loadAnnotation{
	...
	//load annotation in layer
	UIImage *image;
	NSFileManager *fileManager = [NSFileManager defaultManager];
	NSString *fileName = [[NSUserDefaults standardUserDefaults] objectForKey:@"FILE_NAME"];
	NSString *annotationFilePath = [NSString stringWithFormat:@"%@/%@-%d.png",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0],fileName,currentPageIndex+1];
	if ([fileManager fileExistsAtPath: annotationFilePath])
	{
		image = [[UIImage alloc] initWithContentsOfFile:annotationFilePath];
		annotationPage.contents = (id)image.CGImage;
		//annotationPageReverse.contents = (id)image.CGImage;
		[image release];
	}
	else {
		annotationPage.contents = nil;
		//annotationPageReverse.contents = nil;
	}
	annotationFilePath = [NSString stringWithFormat:@"%@/%@-%d.png",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0],fileName,currentPageIndex+2];
	if ([fileManager fileExistsAtPath: annotationFilePath])
	{
		image = [[UIImage alloc] initWithContentsOfFile:annotationFilePath];
		bottomAnnotationPage.contents = (id)image.CGImage;
		[image release];
	}
	else {
		bottomAnnotationPage.contents = nil;
	}
}
...
- (void)dealloc {
	...
	[annotationPageReverse release];
	[bottomPage release];
	[bottomPageShadow release];
	[bottomAnnotationPage release];
	[pageCache release];
...
- (void) setLayerFrames {
	...
	annotationPage.frame = topPage.bounds;
	bottomAnnotationPage.frame = bottomPage.bounds;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([self touchedPrevPage] && [self hasPrevPage]) {		
		...
		touchIsActive = NO;
	if ([delegate respondsToSelector:@selector(leavesView:touchesBeganInPageAtIndex:)])
		[delegate leavesView:self touchesBeganInPageAtIndex:self.currentPageIndex];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	if (!touchIsActive){
		if ([delegate respondsToSelector:@selector(leavesView:touchesEndInPageAtIndex:)])
			[delegate leavesView:self touchesEndInPageAtIndex:self.currentPageIndex];
		return;
	}
	touchIsActive = NO;
